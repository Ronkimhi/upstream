...
def main() -> int:
    ...
    for ln in todays:
        ...
        # A NOTE-kind line whose command names a dive records a stop, not a result:
        # PENDING_DATA on a stale series, BLOCKED on a missing audit. No stock file was
        # written, so there is no verdict, clock or grade to name, and demanding them
        # failed the first dive that DID complete on 2026-09-06 nine times over three
        # peers' stop notes (backlog gate-scans-non-dive-notes). Only RUN and AMEND lines
        # are dive results.
        if kind == "NOTE":
            continue
