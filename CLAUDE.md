...
3. **Claim before executing (double-run guard; refined after the 2026-08-29 race).** More than one session can be live at once, and multiple sessions SHOULD watch the artifact (redundancy — sessions die). The serialization is the claim, in this exact order: (a) `git pull --rebase` if a remote exists and is reachable; (b) READ the tail of `data/ledger.md` — if a drain-claim line for the entry id already exists, stand down without committing anything; (c) append your own `... | NOTE | drain-claim <entry ids> | by: <session> ...` line and COMMIT it; (d) push race-safe when pushable. Arbitration: sessions sharing one working tree are ordered by the first claim COMMIT in local history; separate clones are ordered by the push rebase (a lost rebase that pulls in a rival claim for the same ids = theirs, skip). Only execute ids your own claim holds.
