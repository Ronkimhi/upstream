...
- **The price plane is SINGLE-SOURCE, and that is stated rather than assumed (2026-08-29).** `tools/acis/dual_source.py` was written to fetch every capital-action number from two independent sources (yfinance + stooq) and to mark disagreement `DISPUTED`. Stooq answers the GitHub Actions venue with an HTML robots page rather than CSV — verified twice on this date, first with the EDGAR user agent and again with a full browser header set — so the second leg has never answered in this repo's life and every price on disk is `SINGLE_SOURCE` from yfinance alone. The failure was invisible because both legs swallowed their errors and the price control probe fails closed; each leg now records why it did not answer, in `market/<T>.json.legs`. Two consequences are load-bearing: a dive resting on a `SINGLE_SOURCE` or `DISPUTED` price must carry a `price_source_note` saying so (gate-enforced), and `compare_prints` will not call two prints from DIFFERENT sessions an agreement, because a stale peer print is not a second reading of today's close. Restoring a real second source needs an API key and therefore a decision from Ron; until then the guarantee is documented as unmet rather than implied.
