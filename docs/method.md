...
**The occurrence is dated.** Every signal carries an `occurrence` block: `{kind, anchor_date, window, label}` with `kind ∈ HAPPENED | UNDERWAY | SCHEDULED`. HAPPENED = a discrete past event; UNDERWAY = a structural shift in progress (anchor on its start date); SCHEDULED = a known future event (policy effective date, ruling, launch, corporate act) that has not happened yet — forward visibility is part of the hunt, not an afterthought. `anchor_date` is required for all kinds and snaps when imprecise: month known → 1st of month; quarter/half → midpoint; year only → YYYY-07-01; the honest range goes in `window` (e.g. "H1 2027 ruling window"). The label is the short caption the cortex shows. Since 2026-09-06 every card also carries `short_title` (at most 44 characters): the card's own caption for the field beside its hub, an abbreviation of `title` and never a new claim; a card without one is printed as its title up to the colon.
...
**Instrument heat (amended 2026-09-13).** A link carrying `price_instruments` (§4, the price test) is scored twice. The link's `impact` is shared. `heat.instrument{as_of, crowdedness, capture, verdict, ticker_refs}` scores the instrument expression on the same bar as the issuer expression: crowdedness from PCS on the instrument's own ticker (`data/market/<T>.json.pcs`, requested and never remembered), its AUM and flows, and coverage counts, with the same anchors; capture is what a holder keeps of a move in the scarce price: roll yield or contango at the fund's tenor, the expense ratio, tracking, all cited from the fund's own documents. `heat.instrument.verdict` is `band_for(impact, instrument crowdedness)`, computed and never written. Both scores are explicit NULL with a basis until the instrument's market file lands, and then the verdict is null. `heat_health.instruments{examined, scored, pending, errors}` counts links with instruments. An instrument block scored after the chain's heat run carries its own `as_of` and leaves `heat_as_of` and the issuer scores on the day they were written; `tools/check_heat.py` treats that `as_of` as a heat write on the chain, bound by the same same-day ledger, calibration and health rules, and a full heat run re-scores the instrument on its own date. The link's own `verdict` and `money_corner` still describe the issuer expression and are unchanged. Why: on 2026-08-30 `hormuz-maritime/crude-tanker-tonnage` scored crowdedness 84 on the shipping stocks and closed OVER_CROWDED while the fund that holds the freight rate itself was never measured; one crowdedness number cannot describe a link with two expressions.
...
**The price test (amended 2026-09-13).** Every link answers what price its scarcity sets and whether that price can be held directly. `scarce_price{name, unit, published_by, evidence[]}` names the price and who publishes it (a Baltic Exchange route, a hull war-risk rate, a crack spread, a charter rate), or `{name: null, basis}` when the link sets no price. `price_instruments[]` lists every listed instrument that holds that price directly, each with `ticker`, `exchange`, `kind` (`ETF | ETN | COMMODITY_POOL | PHYSICAL_TRUST | FUTURES | INDEX_NOTE`), `holds` copied verbatim from the issuer's own page, `tenor`, `expense_ratio`, `aum{value, as_of}` where published, and dated official `identity_evidence[]` in the §6A shape. `instrument_search{searched_at, queries[], result, boundary}` records the searches, so an empty list is a finding and not an omission: `FOUND`, `NONE_FOUND` (searched, nothing listed holds it), or `NOT_APPLICABLE` (the link sets no price). Required on every link whose bottleneck is HIGH or CHOKE_POINT; recorded on the rest when the answer is known. Why: on 2026-08-30 `hormuz-maritime/crude-tanker-tonnage` was scored impact 95 on a freight rate that had risen more than tenfold, crowdedness 84 on the shipping stocks, and closed OVER_CROWDED, while the fund that holds that rate (BWET, 90% TD3C freight futures) was on no map at all. The instrument is the position in front of the operating companies: they capture the price diluted by charter cover, costs and their own multiples; it captures the price net of roll and fees. A map that lists only companies is a map of companies, not of the price. The instrument is scored as its own expression (§3, instrument heat) and is never profiled or dived: §6A and §7 are shaped for operating companies. Chains created before 2026-09-14 warn until backfilled; a chain written or refreshed from 2026-09-14 fails closed (the day after the gate landed, so runs already in flight on 2026-09-13 warn). `tools/check_chain.py` holds it.
...
- WATCH requires non-empty triggers `{metric, level, direction}`, **and (Ron, 2026-09-03; re-anchored 2026-09-08) a would-buy zone `{low, high, basis, as_of}`**: the band at which this file's own **bear case** still clears — not the band that closes its gap table to the base case. Basis names in one line the bear-case row that clears at that price. `high` MAY sit at or above the price the dive reasoned from; when it does the dive records `zone_contains_spot: true`, which is the honest reading of "the bear case clears here" and hands the verdict to the upward check below. Or `would_buy_zone: null` with `would_buy_basis` naming the cap that binds at any price (grade C, grade NULL, the independence test, the pre-mortem). A cap is a real answer; a zone that is merely hard to draw is drawn.
  **Why re-anchored (Ron, 2026-09-08).** Under the 2026-09-03 convention every zone in the corpus closed to its own BASE case, and the gate separately refused any `high` at or above spot. The two rules together meant a WATCH could not contain today's price on the day it was written, by construction, and they put nine of ten drawn zones between 10.4% and 163.6% below spot (ONT's at 163.6%). None of the thirteen WATCH names has ever traded inside its own zone. A price nobody expects to see is a refusal wearing a number. A bear-case-survived floor is a level that can actually be reached, which is what makes a WATCH a plan rather than a no.
- TOO_LATE requires the "what is priced in" decomposition to show it by **either** route (amended 2026-09-08): multiple expansion that ate the runway (story or numbers), **or** the price having already travelled the distance this chain's own scenario said it would, with that scenario named. Auto-creates a shadow row.
  **Why the second route.** Six consecutive dives declined TOO_LATE and Stocky's log named the cause: the word could only describe a name whose multiple EXPANDED, while this funnel keeps sourcing names whose multiples CONTRACTED because earnings arrived faster than the re-rating. 688425.SS fell 29.17% over twelve months against a 1.65% earnings decline, so its trailing P/E went 20.37x to 14.67x and there was no expansion left to decompose. A third of a closed vocabulary that no evidence can ever reach is not a high bar, it is a dead word.
...
So: a dive whose earnings-quality grade is A or B, whose independence test is answered in all three parts, whose net gap exceeds its own `horizon_spread`, and whose price sits inside its own would-buy zone must carry a **`watch_basis`** naming in one line what still caps it. A cap it cannot name means the file's own evidence supports INVESTABLE, and WATCH is then the wrong word. This does not lower any bar above: it requires the analyst to say which bar is binding, the same discipline `would_buy_basis` already requires of a null zone.

**The clock is chosen, not defaulted (Ron, 2026-09-08).** All 14 dives to date are labeled COMPOUNDER and none EVENT, on a machine whose premise is a dated occurrence and the un-crowded links behind it. A dive anchored to a dated occurrence with a live scenario clock is **EVENT**; COMPOUNDER is for a name held through the cycle regardless of the occurrence. `tools/check_analyst.py` warns when a dive on a chain with scenarios files COMPOUNDER without saying why, because grading an event-driven name on the patient-entry framework is how "good business, wait for a dip" becomes the only sentence the machine can write.

**Admission lapse (Ron, 2026-09-14).** A campaign-era FINAL dive is admitted once, through the exact O1/mapping/screen chain this section states (points 1-7 above), and nothing re-derives that afterward on every later gate run. A placement's fresh-context audit can FAIL later, after Stocky already closed a verdict on the role it qualified — the case this exists for is `data/stocks/WSP.TO__data-center-moratoria.json`, FINAL INVESTABLE on the WSP-GLOBAL placement for `permitting-and-environmental-consultancy`, where no readable source states WSP's data-center permitting role. Deleting the dive would erase a real verdict on a real question; leaving it silently at its old verdict would misstate what evidence currently supports. Neither is correct, so: an `admission_lapse{date, reason, audit_ref, prior_verdict, restore_when}` object records the downgrade — `date` when the lapse was recorded, `reason` naming the missing evidence in one substantive line, `audit_ref` naming the failing audit exactly (the mapping file and the placement or link it failed on), `prior_verdict` the verdict the dive held before the lapse (one of this section's closed vocabulary), and `restore_when` what a source would have to show to restore admission. A dive carrying a well-formed `admission_lapse` downgrades its own `verdict` to **WATCH** and keeps `status: FINAL`; every other requirement in this section — the gap table, the earnings-quality veto, the independence test, the completed red team — still applies unchanged, because the lapse is about ADMISSION, not about whether the analysis itself was done or is still sound. The dive stays visible naming the gap rather than disappearing or silently misstating its support, exactly like every other amendment in this file: nothing is ever un-said. `tools/check_analyst.py` reports the admission findings a well-formed WATCH lapse excuses as a lapse, not a failure — printed, not hidden — and fails a lapse recorded on any other verdict, a malformed lapse, or a lapse whose named gap has since closed (the underlying admission now resolves cleanly, so the object is stale and must be removed: an admission that actually passes while a lapse is still present is itself a finding). A lapsed dive stops counting toward a campaign's O1 FINAL-verdict coverage (section 6A) the moment its issuer's profile is demoted from O1 to O2 to match — `tools/check_campaign.py`'s O1 and O1-FINAL counts are both computed from `opportunity_tier` alone, so a profile that is no longer O1 was never in that denominator to begin with, and the lapse mechanism needs no carve-out of its own there.
...
- **Shadow book**: every TOO_LATE verdict, every DISMISSED signal, and since 2026-09-13 every OVER_CROWDED link call writes rows (spot + date). A link writes one row per representative listed name (its example tickers, then its ACTIVE placements, capped at five) and one per price instrument, at the heat date, through `tools/shadow_heat.py` in the heat postlude; Actions reprices at +90d vs SPY; `RIGHT` means standing aside was correct (underperformed SPY). The machine's "no" gets graded. Hit rates are reported per origin, never pooled, because Stocky's TOO_LATE number and Ember's OVER_CROWDED number answer different questions; a link is read as the median of its issuer rows with its instrument row beside it.
- **Front door (2026-09-13)**: the page's Board opens with the three biggest opportunities by link size, `impact × capture × (100 − crowdedness) / 10^4` per expression with the link's best expression counting, computed by `tools/opportunities.py` at build time from the heat blocks and never written by hand. Each card names the best thing on the link (a verdict, a selected or screened name, the fund that holds the price, or the first example ticker) and how far the machine has gone. Every number on a card names its file and `tools/check_opportunities.py` re-reads it; the block prints how many links it ranked and how many it could not. The Saturday digest copies the three rows as `top3`, so the ranking has a history.
