...
4. **Triage artifact-changed notifications before spending a read.** The version string in the notification (`1788027370-1843`) is a **unix timestamp** plus a hash. Measured across eight versions on 2026-08-29:
   - **Lower timestamp than the last version you verified = an out-of-order replay.** Skip it entirely; the read is pure waste. Two of the day's notifications were replays of versions already superseded.
   - **A click republish is visible as a gap.** The page is republished from the browser with only the queue block changed, so its `built_at` does not move: version `1788027370` published at 18:16:10Z carried `built_at 15:37Z`, a 2h39m gap. Every build republish sat within ~2 minutes of its own `built_at` (18:22:46Z / 18:21Z, 18:39:04Z / 18:38Z).
   - So: **version timestamp far ahead of the page's `built_at` = a real click, drain it. Version timestamp ≈ `built_at` = someone's build republish, no action.** This is stronger than "did `built_at` change", because it needs no memory of the previous value.
   Between three sessions this cost five wasted ~395KB reads before anyone worked it out.
