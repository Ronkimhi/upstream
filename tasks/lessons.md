...
## 2026-08-30: Model tier was a promise until it failed twice

- The lesson above was prose only: no file in `.claude/agents/` named a model, so every invocation silently inherited whatever model the calling session happened to be running. First failure: the lesson had to be written at all, which means it had already been broken once. Second failure: Ron reported a stock search (`run deepdive`/`run redteam`) ran on an unnecessarily expensive model, with nothing in the machine to have stopped it.
- **The promotion.** `tools/model_tiers.py` is now the single source of truth for which tier (`fast` = `claude-haiku-4-5-20251001`, `strong` = `claude-sonnet-5`) each governed command requires, keyed the same way `tools/agent_registry.py` already resolves a concrete invocation to its command-table shape. Opus and Fable are never defaulted anywhere in that table on purpose: defaulting to the biggest available model is exactly the failure being closed, escalation is something Ron asks for in the turn that needs it.
- Ember, Stocky, Cass, and Adam each own only commands that sit in one tier, so their `.claude/agents/*.md` frontmatter now pins `model: claude-sonnet-5` directly — a real, functional default the Agent tool reads. Nell, Tally, Atlas, and Sieve straddle both tiers within one contract (e.g. Atlas's `run universe` is mechanical, `run chain` is judgment), so a static pin can't serve them; `CLAUDE.md`'s Commands section now instructs every session to look up the tier before calling the `Agent` tool and pass it explicitly, the same way `CLAUDE.md` already carries the Ralph-loop backlog/lessons architecture into every session without Ron repeating it.
- The ledger line schema gained a `model:` field so this is checkable, not just instructed.
- **Enforced by `tools/check_model.py`**, same `Report`/denominator style as `check_machine.py`: command-table coverage, frontmatter agreement, and ledger evidence. Advisory on day one (zero ledger lines carry `model:` yet), same stated reason as `check_machine.py` — a gate that fails on its first run gets routed around. `tools/tests/test_model.py` certifies it as falsifiable.
...
## 2026-08-31: A ledger line records that a stage RAN, not that it SUCCEEDED

- `tools/campaign_board.py` named `run universe-audit munitions-replenishment` as the next action on four consecutive routine fires. Each fire stood down without writing anything, reasoning from a ledger line that the stage had already been carried out at HEAD. The file on disk had **no `audit` block at all** and status `ACTIVE`. Four fires produced four NOTE lines and zero progress, and the third escalated.
- The reasoning error is specific and worth naming: the routine treated the append-only ledger as a record of OUTCOMES. It is a record of ATTEMPTS. Every gate in this repo already knows the difference, which is why they all read disk and recompute rather than reading `data/ledger.md` for anything except proof that a run declared itself.
- **The rule: when the board and a ledger narrative disagree, the board wins.** The board is derived from disk by the same evidence ladder the campaign gate uses. A ledger line is prose written by whoever ran the command, including a session that crashed after writing it. Before standing down on "already done", open the file and look for the thing that stage was supposed to write.
- Cheap general form: a stand-down needs the same evidence as a claim. "Nothing to do here" is an assertion about disk state and must be checked against disk state.
- Enforceable, and this is the second failure of the class (the first was a session-limit death leaving nine agents' work unwritten while their ledger intent existed): `tools/campaign_board.py` should print, beside each next-command, the exact file predicate that would make it stop being next. A stander-down could then be asked to quote it. Not built yet; recorded here so the next occurrence promotes it rather than re-diagnosing it.

## 2026-09-01: The blockers were gates refusing data already on disk

- Ron asked why the machine had so many data blockers and still could not say where to invest. Measured: 67 of 103 profiles BLOCKED, and 62 of those had `fundamentals` and a `quality` block on disk under the dashed filename the fetcher writes. They were refused by two things that were never about data: a profile-gate rule written before the vendor leg existed (INFERRED needs `official_source: true`, method §6A), and a writer that looked for `data/market/AFCONS.NS.json`, found nothing, and declared the file never fetched while `AFCONS-NS.json` sat there with 20 of 20 fields.
- Two further stalls had the same shape. Four censuses waited on a whole-map fresh-context audit that no cloud session could run, so screens on 84 listings were refused for want of a stage that examines all 84 before any one is used. And the SEC fundamentals leg read only `us-gaap` and only 10-K/10-Q, so an IFRS 20-F filer with a CIK returned zero fields and, because it had a CIK, never fell through to the vendor leg.
